...
$nm_estilo = "SELECT nm_estilo FROM tb_estilo
WHERE id_estilo = '" . $id_estilo . "'";
$stmt_nome =  $conn->prepare($nm_estilo);
$stmt_nome->execute();
$estilo = $stmt_nome->fetch(PDO::FETCH_ASSOC);

$sql_id_publicacao = "SELECT fk_id_publicacao FROM tb_estilo_publicacao
WHERE fk_id_estilo = '" . $id_estilo . "'";
$stmt_estilo = $conn->prepare($sql_id_publicacao);
$stmt_estilo->execute();
$publicacoes = $stmt_estilo->fetchAll(PDO::FETCH_ASSOC);

if (count($publicacoes) > 0) {
    $ids_publicacoes = array_column($publicacoes, 'fk_id_publicacao');

    $placeholders = implode(',', array_fill(0, count($ids_publicacoes), '?'));
    $pubs = " SELECT 
    p.id_publicacao,
...
LEFT JOIN tb_estilo_publicacao ep
ON ep.fk_id_publicacao = p.id_publicacao
WHERE u.id_usuario != $id_usuario
AND ep.fk_id_estilo = $id_estilo
...
<head>
    ...
    <title>EstiloEscolhido</title>
    <script src="Pesquisa.js"></script>
    <script src="scripts/modal.js" defer></script>
    <link rel="stylesheet" href="EstiloEscolhido.css">
    <link rel="stylesheet" href="modoEscuro.css">
        ...
        <header class="top-bar">
            ...
            <div class="l2">
                <div class="pesquisa" id="pes" class="pesquisa compact" onclick="pesquisa()">
                    ...
                    <i class="bi bi-search" alt="Buscar" class="search-icon" readonly> </i>
                </div>
                ...
                <div class="popup-menu hidden">
                    <div onclick="window.location.href='#tudo'">Tudo</div>
                    <div onclick="window.location.href='#seguindo'">Seguindo</div>
                    <!-- <div onclick="window.location.href='#meus-relatos'">Meus Relatos</div> -->
                </div>
            </div>

            <div class="l3">
                <h4>Aqui está tudo que foi publicado em <?php echo $estilo['nm_estilo']; ?></h4>
            </div>
        </header>

        <div class="feed">
            <?php if (!empty($publicacao)) { ?>
            ...
            <?php } else { ?>
                <div class="msg">
                    <h1>Ainda não há publicações com esse estilo</h1>
                </div>
            <?php } ?>
        </div>
